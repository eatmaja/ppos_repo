...
<table id="suspended_sales_table" class="table table-striped table-hover">
  ...
  <tbody>
    <?php foreach ($suspended_sales as $suspended_sale) : ?>
      <tr>
        <td><?= esc($suspended_sale['doc_id']) ?></td>
        <td>
          <?= isset($suspended_sale['sale_time']) && $suspended_sale['sale_time']
            ? date($config->dateformat, strtotime($suspended_sale['sale_time']))
            : '-' ?>
        </td>
        <?php if ($config->dinner_table_enable) : ?>
          <td><?= esc($suspended_sale['dinner_table_name'] ?? '-') ?></td>
        <?php endif; ?>
        <td>
          <?php
          if (!empty($suspended_sale['customer_id'])) {
        ...
        <td>
          <?php
          if (!empty($suspended_sale['employee_id'])) {
